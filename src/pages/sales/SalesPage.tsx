...
import EmailInvoiceModal from '../../components/shared/EmailInvoiceModal';
...
const SalesPage = () => {
  ...
  const [emailing, setEmailing] = useState<SalesInvoice | null>(null);
  ...
  const openCreate = (prefillCustomer?: string, appointmentId?: string) => {
    form.reset({
      // always suggest a number - staff can edit it before saving if they prefer their own format
  ...
  return (
    <div className="sales-page">
      ...
      <div className="table-wrap">
        <div className="table-header">
          ...
          <div className="table-actions">
            ...
            {(filterCustomer || filterStatus || filterFrom || filterTo || filterOverdue) && (
              <button
                type="button"
                className="btn-ghost"
                onClick={() => {
                ...
                <tr key={inv.id}>
                  ...
                  <td className="row-actions">
                    ...
                    <button
                      ...
                      title={inv.emailSentAt ? `Last sent ${fmtDate(inv.emailSentAt)}` : 'Email this invoice to the customer'}
                    >
                      {inv.emailSentAt ? 'Re-email' : 'Email'}
                    </button>
                  </td>
                </tr>
              ))
      ...
      <EmailInvoiceModal
        ...
        invoiceId={emailing?.id ?? ''}
        invoiceNumber={emailing?.invoiceNumber ?? ''}
        customerName={emailing ? customerName(emailing.customerUserId) : ''}
        customerEmail={emailing ? (customers.find((c) => c.userId === emailing.customerUserId)?.email ?? '') : ''}
        alreadySent={!!emailing?.emailSentAt}
