...
const InventoryPage = () => {
  ...
  // resolver is cast because z.coerce.number() makes zod infer the input type as `unknown`,
  // which TS can't match to PartFormData's `number` fields. the runtime behaviour is correct.
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const editForm = useForm<PartFormData>({ resolver: zodResolver(partSchema) as any });
